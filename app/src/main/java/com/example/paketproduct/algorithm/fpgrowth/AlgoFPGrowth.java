 package com.example.paketproduct.algorithm.fpgrowth;
 ...
 */
 ...
 import com.example.paketproduct.algorithm.pattern.Itemset;
 import com.example.paketproduct.algorithm.pattern.Itemsets;
 import com.example.paketproduct.algorithm.tools.ConvertFromTabular;
 import com.example.paketproduct.algorithm.tools.MemoryLogger;
 ...
 public class AlgoFPGrowth {
    ...
    private Map<String, Integer> itemAndInit = new HashMap<>();
      ...
      */
     public Itemsets runAlgorithm(AssetManager input, String output, double minsupp) throws FileNotFoundException, IOException {
         // record start time
         ...
         //convert from tabular
         ConvertFromTabular converter = new ConvertFromTabular(input);
         final List<List<Integer>> dataset = converter.getDataset();

         //fill item and initial
         this.initAndItem = converter.getInitAndItem();
         this.itemAndInit = converter.getItemAndInit();

         // (1) PREPROCESSING: Initial database scan to determine the frequency of each item
         // The frequency is stored in a map:
         //    key: item   value: support
         final Map<Integer, Integer> mapSupport = scanDatabaseToDetermineFrequencyOfSingleItemsTabular(dataset);
         ...
         FPTree tree = buildFPTree(dataset, mapSupport);
     ...
     private FPTree buildFPTree(List<List<Integer>> dataset, Map<Integer, Integer> mapSupport) throws IOException {
         FPTree tree = new FPTree();

         // read the file
         for (List<Integer> transaksi : dataset){

             List<Integer> transaction = new ArrayList<>();
             for (Integer item : transaksi){
 ...
 //					}
     ...
     public  Map<Integer, Integer> scanDatabaseToDetermineFrequencyOfSingleItemsTabular(List<List<Integer>> dataset)
             throws FileNotFoundException, IOException {
         // a map for storing the support of each item (key: item, value: support)
         Map<Integer, Integer> mapSupport = new HashMap<>();
         //Create object for reading the input file

         for(List<Integer> transaction : dataset){
